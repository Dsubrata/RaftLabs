...
{
    internal class CosmosDb<T> : BaseDatabase<T> where T : class
    {
        private readonly string connectionString;

        public CosmosDb(string connectionString)
        {
        ...
        {
            using var dbContext = new CosmosDbEntities(connectionString);
        ...
        {
            using DbContext dbContext = new CosmosDbEntities(connectionString);
            DbSet<T> dbSet = dbContext.Set<T>();
        ...
        public override T Get(object id)
        {
            using DbContext dbContext = new CosmosDbEntities(connectionString);
            DbSet<T> dbSet = dbContext.Set<T>();
            return dbSet.WithPartitionKey("").FirstAsync().Result;
        }
